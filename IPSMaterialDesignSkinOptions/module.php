...
class IPSMaterialDesignSkinOptions extends IPSModule
{
  ...
  public function ApplyChanges() {
    ...
    $this->Log("");
    // M�gliche Skin-Werte laden und dem Profil zuweisen
  ...
  private function ApplyTheme($_ApplySkin=false, $_ApplyAccent=false, $_ApplyCardShadow=false) {
    ...
    /*
      ...
      if (!file_exists($icon_path) or !is_dir($icon_path) ) {
	      $this->Log("Verzeichnis (".$icon_path.") f�r die eingef�rbten Icons existiert nicht.");
	      return false;
      }
      if (!is_writeable($icon_path)) {
	      $this->Log("Verzeichnis (".$icon_path.") f�r die eingef�rbten Icons ist nicht beschreibbar.");
        return false;
      }
